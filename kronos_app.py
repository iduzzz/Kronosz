...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/forecast_combined":
            ...
            if GROK_KEY:         engines_to_run.append(("Grok-3",           lambda: run_grok2(symbol, interval, klines, ticker)))
            if GEMINI_KEY:       engines_to_run.append(("Gemini 2.5 Flash", lambda: run_gemini(symbol, interval, klines, ticker)))
            if MISTRAL_KEY:      engines_to_run.append(("Mistral Large",    lambda: run_mistral(symbol, interval, klines, ticker)))
        ...
        if parsed.path == "/forecast":
            ...
            try:
                if engine == "gpt4o":
                    ...
                    analysis = run_mistral(symbol, interval, klines, ticker)
                elif engine == "claude":
            ...
            result = {
                ...
                "engine": {"gpt4o":"GPT-4o","grok2":"Grok-3","gemini":"Gemini 2.5 Flash","mistral":"Mistral Large","claude":"Claude Haiku 4.5","kronos":"Kronos-base 102M"}.get(engine, engine),
                "price":    float(ticker["lastPrice"]),
                "chg":      float(ticker["priceChangePercent"]),
